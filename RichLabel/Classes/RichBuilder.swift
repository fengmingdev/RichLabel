...
typealias RichFilterPredicate = ((String) -> Bool)

struct RichBuilder {

    static func createElements(type: RichType, from text: String, range: NSRange, filterPredicate: RichFilterPredicate?) -> [ElementTuple] {
        switch type {
    ...
    static func createURLElements(from text: String, range: NSRange, maximumLength: Int?) -> ([ElementTuple], String) {
        ...
        for match in matches where match.range.length > 2 {
            ...
            guard let maxLength = maximumLength, word.count > maxLength else {
                ...
                let element = RichElement.create(with: type, text: word)
            ...
            let element = RichElement.url(original: word, trimmed: trimmedWord)
    ...
    private static func createElements(from text: String,
                                            for type: RichType,
                                                range: NSRange,
                                                minLength: Int = 2,
                                                filterPredicate: RichFilterPredicate?) -> [ElementTuple] {
