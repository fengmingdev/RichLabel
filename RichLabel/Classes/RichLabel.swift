...
public protocol RichLabelDelegate: class {
    func didSelect(_ text: String, type: RichType)
}

public typealias ConfigureLinkAttribute = (RichType, [NSAttributedString.Key : Any], Bool) -> ([NSAttributedString.Key : Any])
typealias ElementTuple = (range: NSRange, element: RichElement, type: RichType)

@IBDesignable open class RichLabel: UILabel {
    
    // MARK: - public properties
    open weak var delegate: RichLabelDelegate?
    
    open var enabledTypes: [RichType] = [.mention, .hashtag, .url]
    ...
    open var customColor: [RichType : UIColor] = [:] {
        didSet { updateTextStorage(parseText: false) }
    }
    open var customSelectedColor: [RichType : UIColor] = [:] {
        didSet { updateTextStorage(parseText: false) }
    }
    ...
    open func handleCustomTap(for type: RichType, handler: @escaping (String) -> ()) {
    ...
    open func handleOutsideTap(_ handler: @escaping () -> ()) {
        outsideTapHandler = handler
    }
    
    open func removeHandle(for type: RichType) {
        switch type {
        case .hashtag:
    ...
    // MARK: - customzation
    @discardableResult
    open func customize(_ block: (_ label: RichLabel) -> ()) -> RichLabel {
        _customizing = true
        block(self)
        _customizing = false
        updateTextStorage()
    ...
    internal var customTapHandlers: [RichType : ((String) -> ())] = [:]
    
    internal var outsideTapHandler: (() -> ())?
    ...
    lazy var RichElements = [RichType: [ElementTuple]]()
    ...
    fileprivate func updateTextStorage(parseText: Bool = true) {
        ...
        // clean up previous Rich elements
        ...
        if parseText {
            clearRichElements()
            let newString = parseTextAndExtractRichElements(mutAttrString)
    ...
    fileprivate func clearRichElements() {
        selectedElement = nil
        for (type, _) in RichElements {
            RichElements[type]?.removeAll()
        }
    }
    ...
    fileprivate func addLinkAttribute(_ mutAttrString: NSMutableAttributedString) {
        ...
        for (type, elements) in RichElements {
    ...
    fileprivate func parseTextAndExtractRichElements(_ attrString: NSAttributedString) -> String {
        var textString = attrString.string
        var textLength = textString.utf16.count
        var textRange = NSRange(location: 0, length: textLength)
        
        if enabledTypes.contains(.url) {
            let tuple = RichBuilder.createURLElements(from: textString, range: textRange, maximumLength: urlMaximumLength)
        ...
        for type in enabledTypes where type != .url {
            ...
            let hashtagElements = RichBuilder.createElements(type: type, from: textString, range: textRange, filterPredicate: filter)
            RichElements[type] = hashtagElements
        }
    ...
    fileprivate func element(at location: CGPoint) -> ElementTuple? {
        ...
        for element in RichElements.map({ $0.1 }).joined() {
    ...
    fileprivate func didTap(_ element: String, for type: RichType) {
    ...
    fileprivate func didTapOutside() {
        guard let outsideTapHandler = outsideTapHandler else {
            return
        }
        outsideTapHandler()
    }
...
extension RichLabel: UIGestureRecognizerDelegate {
